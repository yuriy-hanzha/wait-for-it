...
usage()
{
    cat << USAGE >&2
Usage:
    $WAITFORIT_cmdname url [-s] [-t timeout] [-- command args]
    -u URL | --url=URL          Url under test
                                Alternatively, you specify a url as host:port/query-params
...
wait_for()
{
    ...
    do
        if [[ $WAITFORIT_ISBUSY -eq 1 ]]; then
            curl -f -s $WAITFORIT_URL
            WAITFORIT_result=$?
        else
            (echo > curl -f -s $WAITFORIT_URL) >/dev/null 2>&1
            WAITFORIT_result=$?
        fi
